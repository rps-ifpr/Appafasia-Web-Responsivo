<?php
	...
	if(isset($inicio) && !empty($inicio)){
		if($usuario == 1){
			...
			$sql = "select * from atividades left join usuario on atividades.usuario = usuario.id where andfonoaudiologo = ".$usuario." and reacao = ".$reacao." and data >= STR_TO_DATE('".$inicio."', '%Y-%m-%d') AND data <= STR_TO_DATE('".$fim."', '%Y-%m-%d') order by data;";
		}
	}else{
